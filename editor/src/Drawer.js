(function($, namespace, pluginsNamespace, util, texts) {
     ...
     * @param zoomFactor {number}  ZoomFactor integer (editör kaç x büyük çalışılmış)
     *
     * @memberof DrawerJs
     * @constructor
     */
    ...
    Drawer.prototype._startEditing = function() {
        ...
        //var x = this.$imageElement.position();
        //console.log(x);
        //this.top = x.top;
        //this.left = x.left;
        ...
        $('#canvas-editor').append(this.$canvasEditContainer);
    ...
    Drawer.prototype.setColor = function(newColor, newOpacity) {
        ...
        if (activeObject) { // objects background
            console.log("activeObject type: " + activeObject.type + " - activeColor: " + this.activeColor);
            if(activeObject.type == "TextZone")
            {
                activeObject.set('fill', this.activeColor);
                activeObject.set('backgroundColor', '#000000');
            }else{
                activeObject.set('fill', this.activeColor);
            }
        } else { // canvas background
            console.log("activeObject type: canvas - activeColor: " + this.activeColor);
            this.fCanvas.backgroundColor = this.activeColor;
        } 
     ...
     * @param {number} height
     */
    Drawer.prototype.setSize = function(width, height, zoomFactor) {
        this.width = width;
        this.height = height;
        this.zoomFactor = zoomFactor || 1;

        if (this.fCanvas) {
            this.fCanvas.setWidth(this.width * zoomFactor);
            this.fCanvas.setHeight(this.height * zoomFactor);
        }

        this.$imageElement.css('width', this.width * zoomFactor);
        this.$imageElement.css('height', this.height * zoomFactor);

        if (this.$canvasEditContainer) {
            this.$canvasEditContainer.css('width', this.width * zoomFactor);
            this.$canvasEditContainer.css('height', this.height * zoomFactor);
        }

        // canvas'a zoomFactor adında yeni bir property ekliyoruz
        this.fCanvas.zoomFactor = zoomFactor;
    ...
    /**
     * Set zoomFactor of Drawer
     * @returns {DrawerJs.DrawerApi.getZoomFactor}
     */
     Drawer.prototype.getZoomFactor = function() {
    ...
    Drawer.prototype.getAlign = function() {
        ...
        if (this.aligmentCss['position'] == 'absolute') {
            ...
            currentAlign = 'center';
        } else if (this.aligmentCss['position'] == 'fixed') {
            currentAlign = 'editor';
        }
     ...
     */
    Drawer.prototype.createGrid = function(w, h, isSnap, isOnlyRow) {
        if (this.fCanvas) {
            //if (!w) { w = this.drawer.defaultOptions.gridW; }
            //if (!h) { h = this.drawer.defaultOptions.gridH; }
         this.fCanvas.createGrid(w, h, isSnap, isOnlyRow);
         //this.fCanvas.renderAll();
        }
    };
    ...
    Drawer.prototype.clearCanvas = function() {
        if (this.fCanvas) {
            this.fCanvas.clearCanvas();
            this.fCanvas.backgroundColor = '#000';
            this.fCanvas.renderAll();
        }
    };
     ...
     * Save canvas.
     */
    Drawer.prototype.saveCanvas = function() {
        if (this.fCanvas) {
            this.fCanvas.saveCanvas();
        }
    };
     ...
     */
    Drawer.prototype.colorCanvas = function(color) {
        if (this.fCanvas) {
            this.fCanvas.backgroundColor = color;
            this.fCanvas.renderAll();
        }
    };
     ...
     */
    /*   Drawer.prototype.backgroundImage = function (url) {
        if (this.fCanvas) {
         this.fCanvas.backgroundImage(url);
        }
     }; */
    ...
    Drawer.prototype.activateDefaultPlugin = function() {
        ...
        if ((pluginMode == 'always') || (pluginMode == 'lastUsed') ||
            ...
            if (defaultTool) {
                ...
                this.trigger(this.EVENT_DO_DEACTIVATE_TOOL, [defaultTool]);
            } else {
    ...
    Drawer.prototype.onCanvasModified = function(ignoreOptions) {
        ...
        // image içeriklerinde Tainted canvases may not be exported. hatası veriyor 
        // bu event gereksiz ve iptal edildi
        ...
        if (objProps) {
            for (var propertyName in objProps) {
                if (objProps.hasOwnProperty(propertyName)) {
                    // burası canvas properties
                    //console.log("_updateObjectsControls: " + propertyName);
                    //console.log("objProps[propertyName]: " +  objProps[propertyName]);
